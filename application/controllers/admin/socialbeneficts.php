...
class Admin_Socialbeneficts_Controller extends Base_Controller
{
	...
	{
		// gets the employees id, FIX IT, MUST BE PASSED AS A PARAMETER
		$id = Input::get('id');

		// get the input fields
		$input["startdate"]				= Input::get('startdate');
		$input["stopdate"] 				= Input::get('stopdate');
		$input["down_salaries_days"] 	= Input::get('down_salaries_days');
		$input["received_advances"] 	= Input::get('received_advances');
		$input["received_loans"] 		= Input::get('received_loans');
		$input["others"]				= Input::get('others');
		$input["check"] 				= Input::get('check');
		
		// set the validation rules
		$rules = array(
				'startdate'				=> array('required','date_format:d-m-Y'),
				'stopdate'				=> array('required','date_format:d-m-Y'),
				'down_salaries_days'	=> array('required','numeric','max:999'),
				'received_advances'		=> array('required','numeric','max:99999'),
				'received_loans'		=> array('required','numeric','max:99999'),
				'others'				=> array('required','numeric','max:99999'),
				'check'					=> array('required','max:10'),
			);
		...
		if ($validation->fails()) 
		{
			return Redirect::to('admin/socialbeneficts/pre/'.$id)->with_errors($validation);
		} 
		// validation passed
		else  
		{
			// gets the start and stopdate in order to validate
			$start 	= new DateTime(Input::get('startdate'));
			$stop 	= new DateTime(Input::get('stopdate'));
			...
			if ($start >= $stop)
			{
				...
				// adds the message
				$messages->add('period','La fecha de egreso debe ser mayor a la de ingreso.');

				// redirects with the error
				return Redirect::to('admin/socialbeneficts/pre/'.$id)->with_errors($messages);
			}
